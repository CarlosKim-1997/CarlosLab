...
var relationKeys = ["depends_on", "blocked_by", "supersedes", "implements", "related_to", "resolved_by"];
var reverseKeys = ["blocks", "superseded_by", "implemented_by", "depended_on_by", "replaced_by", "resolved"];
var manifestKeys = ["schema", "governance_version", "areas", "current_state", "supported_schemas"];
var commonKeys = ["schema", "id", "status", "areas", ...relationKeys];
var idRegex = /^(D|INV|C|OQ|T)-[0-9]+$/;
var areaRegex = /^[a-z][a-z0-9-]*$/;
var has = (o, k) => Object.hasOwn(o, k);
var arraysEqual = (a, b) => a.length === b.length && a.every((v, i) => v === b[i]);
function parseYaml(source, path, add) {
  const doc = import_yaml.default.parseDocument(source, { uniqueKeys: true, strict: true });
  ...
  try {
    ...
    add("ERROR", "GOV-SCHEMA-YAML", path, `invalid YAML: ${error.message}`);
    return null;
  }
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    add("ERROR", "GOV-SCHEMA-YAML", path, "expected YAML mapping");
    return null;
  }
  return value;
}
function parseRecord(source, path, add) {
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/.exec(source);
  if (!match) {
    add("ERROR", "GOV-SCHEMA-FRONTMATTER", path, "missing YAML frontmatter");
    return null;
  }
...
function type(item) {
  return item.data.schema === "constraint/v1" ? item.data.id?.startsWith("INV-") ? "INV" : "C" : item.data.schema === "decision/v1" ? "D" : item.data.schema === "open-question/v1" ? "OQ" : item.data.schema === "task/v1" ? "T" : "STATE";
}
...
function detectCycles(items, edges, rule, add) {
  ...
  const visiting = /* @__PURE__ */ new Set(), visited = /* @__PURE__ */ new Set(), reported = /* @__PURE__ */ new Set();
  function visit(id, chain) {
    if (visiting.has(id)) {
      const cycle = [...chain.slice(chain.indexOf(id)), id].join(" -> ");
      if (!reported.has(cycle)) {
        reported.add(cycle);
        add("ERROR", rule, byId.get(id)?.path ?? "", `cycle: ${cycle}`, id);
      }
      return;
    }
...
async function checkGovernance(base = process.cwd()) {
  ...
  const unsafeRequired = /* @__PURE__ */ new Set();
  for (const p of required) {
    if (!await exists(p)) add("ERROR", "GOV-BOOT-MISSING", p, "missing bootstrap file");
    else if (!insideRoot(await realpath(absolute(p)))) {
      add("ERROR", "GOV-BOOT-ESCAPE", p, "authoritative bootstrap file escapes repository");
      unsafeRequired.add(p);
    }
  }
  let manifest = null;
  ...
  for (const def of Object.values(definitions)) for (const folder of def.roots) {
    let realFolder;
    try {
      realFolder = await realpath(absolute(folder));
    } catch (e) {
      if (e.code === "ENOENT") continue;
      throw e;
    }
    if (!insideRoot(realFolder)) {
      add("ERROR", "GOV-SCHEMA-PATH", folder, "structured collection escapes repository");
      continue;
    }
    let entries = [];
    try {
      entries = await readdir(absolute(folder), { withFileTypes: true });
    } catch (e) {
  ...
  discovered.sort();
  const items = [], ids = /* @__PURE__ */ new Map();
  for (const path of discovered) {
    let realFile;
    try {
      realFile = await realpath(absolute(path));
    } catch (e) {
      if (e.code === "ENOENT") {
        add("ERROR", "GOV-SCHEMA-PATH", path, "record link target is missing");
        continue;
      }
      throw e;
    }
    if (!insideRoot(realFile)) {
      add("ERROR", "GOV-SCHEMA-PATH", path, "record escapes repository");
      continue;
    }
    ...
    if (!def) {
      add("ERROR", "GOV-SCHEMA-UNSUPPORTED", path, "unsupported schema", data.id);
      continue;
    }
    ...
    const allowed = /* @__PURE__ */ new Set([...commonKeys, ...data.schema === "constraint/v1" ? ["kind", "overridable"] : []]);
    ...
    if (data.schema === "state/v1") {
      if (has(data, "id")) add("ERROR", "GOV-ID-STATE", path, "State has no ID");
    } else {
    ...
    for (const key of relationKeys) if (has(data, key)) {
      ...
      if (key === "resolved_by") {
        if (typeof data[key] !== "string" || !/^D-[0-9]+$/.test(data[key])) add("ERROR", "GOV-REL-RESOLUTION", path, "resolved_by must be one Decision ID", data.id, data[key]);
